...
class UploadMore extends React.Component {
    ...
    handleSubmit = () => {
        ...
        editStoreImgs(formData).then((result)=>{
            console.log(result)
        })
    }
    ...
    render() {
        return (
            <div className="Upload">
                ...
                <div style={{ border: 'solid 2px #000', display: 'inline-block', borderRadius: '5px', padding: '5px 10px', fontWeight: '900'}} onClick={()=>{
                    this.handleSubmit()
                }}> Test</div>
            </div>
        )
    ...
    componentDidMount = () => {
        const that = this
        getStoreImgs(24).then((result)=>{
            that.state.originalFiles = result[0].act_img_url.split(',')
            let imgArr = result[0].act_img_url.split(',').map(e=>{
                return {
                    id: e,
                    imagePreviewUrl: e
                }
